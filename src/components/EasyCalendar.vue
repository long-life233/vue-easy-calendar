<script setup>
  ...
  const activities = [
    {
      title: "我为家乡代言",
      beginTime: new Date(2022, 4, 1, 0, 0, 0, 0),
      endTime: new Date(2022, 4, 2, 0, 0, 0, 0),
      color: 'red'
    },
    {
      title: "我为家乡代言",
      beginTime: new Date(2022, 4, 3, 0, 0, 0, 0),
      endTime: new Date(2022, 4, 4, 0, 0, 0, 0),
      color: 'red'
    },
    {
      title: "我为家乡代言",
      beginTime: new Date(2022, 4, 5, 0, 0, 0, 0),
      endTime: new Date(2022, 4, 6, 0, 0, 0, 0),
      color: 'red'
    },
    {
      title: "乡村直播大赛",
      beginTime: new Date(2022, 3, 28, 0, 0, 0, 0),
      endTime: new Date(2022, 4, 7, 0, 0, 0, 0),
      color: 'blue'
    },
    {
      title: "网络直播大赛",
      beginTime: new Date(2022, 4, 23, 0, 0, 0, 0),
      endTime: new Date(2022, 4, 27, 0, 0, 0, 0),
      color: 'green'
    },
  ]
  // 根据这周第一天获取这周的活动
  const getWeekActivity = (weekStart) => {
    // 筛选出来的活动
    let filter = activities.filter(item => {
...
<template>
  <!-- 周一 到 周日 -->
  <div class="week-day">
    <div class="flex items-center q-pl-md" v-for="item in weekdays">
      <span v-if="weekOrMonthFlag==='week'" :class="item.isToday?'item-today':''"
        class="q-mr-md">{{item.monthDay}}</span>
      {{item.day}}
      <span v-if="item.isSaturday && weekOrMonthFlag === 'week'"
        style="background-color:rgba(254, 44, 85, 0.1);color: rgba(254,44,85,.9);padding: 0 2px;"
        class="on-right text-weight-bold rounded">周六</span>
      <span v-if="item.isSunday  && weekOrMonthFlag === 'week'"
        style="background-color:rgba(254, 44, 85, 0.1);color: rgba(254,44,85,.9);padding: 0 2px;"
        class="on-right text-weight-bold rounded">周日</span>
    </div>
  </div>
  <!-- 日历 -->
  <div style="display:inline-block;" class="grid-container-wrapper">
    ...
    <div v-if="weekOrMonthFlag === 'week'" class="gird-week">
      <div class="item" :style="`background-color: ${item.isToday?'#F6F8FA':''};`" v-for="(item,index2) in weekdays">
      </div>
      <span class="activity q-pl-lg" :class="activity.classes"
    ...
    <!-- 月视图日历 -->
    <div v-else>
      <div class="grid-month" v-for="(floor,index) in visibleCalendar">
        <div class="item" v-for="(item,index2) in floor">
          <div class="row items-center">
            <span
              :class="[!item.thisMonth?'text-grey-6':'text-weight-medium',item.isToday?'item-today':'']">{{item.monthDay}}</span>
...
<style scoped lang="scss">
  ...
  .activity {
    ...
    &.activity-red {
      ...
      border: 1px solid rgba(236, 73, 73, .9);
